...
bool Rectangle::hit(const Ray& ray, Real tMin, Real tMax, HitRecord& record) const
{
    return false;
}

bool Rectangle::getAABB(AABB& outputAABB) const
{
    outputAABB = AABB(Vector3(x0, y0, k - 0.0001f), Vector3(x1, y1, k + 0.0001f));
    return true;
}