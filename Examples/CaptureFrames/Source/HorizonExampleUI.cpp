...
namespace HE
{
	...
	void HorizonExample::DrawEntityNodeUI(EntityHandle entity)
	{
	...
	void HorizonExample::OnDrawUI()
	{
		...
		if (ImGui::Begin("Inspector", &showInspectorWindow))
		{
			if (selectedEntity != EntityHandle())
			{
				...
				auto& transformComponent = scene->GetEntityManager()->GetComponent<TransformComponent>(selectedEntity);
				if (ImGui::CollapsingHeader("Transform", ImGuiTreeNodeFlags_DefaultOpen))
				{
					...
					if (dirty)
					{
						scene->GetEntityManager()->ReplaceComponent<TransformComponent>(selectedEntity, transformComponent);
					}
				}

				if (auto* component = scene->GetEntityManager()->TryGetComponent<DirectionalLightComponent>(selectedEntity))
				{
					DrawComponentUI<DirectionalLightComponent>("Directional Light", *component);
				}

				if (auto* component = scene->GetEntityManager()->TryGetComponent<SkyLightComponent>(selectedEntity))
				{
					DrawComponentUI<SkyLightComponent>("Sky Light", *component);
				}

				if (auto* component = scene->GetEntityManager()->TryGetComponent<CameraComponent>(selectedEntity))
				{
					DrawComponentUI<CameraComponent>("Camera", *component);
				}

				if (auto* component = scene->GetEntityManager()->TryGetComponent<StaticMeshComponent>(selectedEntity))
				{
